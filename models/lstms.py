...
def bilstm_layer(input_layer, hidden_layer_dim, rnn_layers):
    lstm = None
    for i in range(rnn_layers):
        if i == 0:
            ...
            nested = lstm
        wrapped = LSTM(
            output_dim=hidden_layer_dim, activation='tanh',
...
def bilstm(n_chars, context=10, hidden_layer=128, rnn_layers=1):
    ...
    lstm = bilstm_layer(in_layer, hidden_layer, rnn_layers)
...
def emb_bilstm(n_chars, emb_dim, context=10, hidden_layer=128, rnn_layers=1):
    in_layer = Input(shape=(context * 2,), dtype='int32', name='input')
    emb_layer = Embedding(
        input_dim=n_chars, output_dim=emb_dim, input_dtype='int32',
        name='emb')(in_layer)
    lstm = bilstm_layer(emb_layer, hidden_layer, rnn_layers)
